...
void RobotomyRequestForm::executeAction() const {
	...
	std::srand(std::time(NULL) + getpid());	// unique seed even if called multiple times in the same second
	
	if (std::rand() % 2) {		// Then, informs that <target> has been robotomized successfully 50% of the time.
